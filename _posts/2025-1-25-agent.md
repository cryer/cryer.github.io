...
- 解析`<tool>`标签包围的内容，获取使用到的工具函数名和参数，并且本地调用函数，如果是中断命令工具，则设置需要用户输入y同意才能进行，因为比较危险，工具执行结果用`<observation>`标签包围后一起传入到大模型中，继续这个循环即可
...
![](C:\Users\kurumi\Desktop\post\33.gif)
...
![](C:\Users\kurumi\Desktop\post\34.gif)

### 总结

主要讲解了`Agent`这个技术的底层原理，实则就是大模型通过Agent主流程调用工具，但是就可以有比单纯的Chat更强的交互感，算是提示词工程的一次进化，由`prompt engineering 提示词工程`进化成`agent engineering 智能体工程`。且随着可用工具的增加，`Agent`可以变得非常强大，比如网络搜索工具，可以联机搜索更多的信息，RAG本地知识库工具，可以进一步增强提示词上下文信息，提高模型输出质量。以及`MCP`协议，专门标准化大模型和工具之间的接口，使得各种软件都可以作为工具，比如支持MCP的地图软件，可以作为可用工具被大模型调用，使得大模型可以为你规划路线等。并且工具类型和大模型类型的倾向不同，不同`Agent`的适用领域也不同，金融领域，编程领域，日常生活领域等等不同的`Agent`可以更具针对性。
